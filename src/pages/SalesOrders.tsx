...
export default function SalesOrders() {
  ...
  const orders = useDataStore((s) => s.salesOrders);
        ...
        {rows.map((o) => (
          <Card key={o.id} className="p-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button variant="secondary" onClick={() => setNotify(o)}><Bell className="h-4 w-4" /> Notify</Button>
              <Button variant="secondary" onClick={() => setInvoice(o)}><FileText className="h-4 w-4" /> Invoice</Button>
            </div>
          </Card>
        ))}
